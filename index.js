...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('Vin Bot', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Vin_Code', 'yellow'))
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            ...
            await aruga.sendTextWithMentions(event.chat, `Hello, Selamat Datang @${event.who.replace('@c.us', '')} \n\nSilahkan Baca Deskripsi Ya Kak, Semoga Betah. Untuk Memulai Bot ketik #help✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
			await aruga.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await aruga.sendTextWithMentions(event.chat, `Selamat Tinggal @${event.who.replace('@c.us', '')}, Kami Merindukan Mu✨`)
        }
    })
